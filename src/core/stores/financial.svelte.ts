...
import type { Country, ProfitCenter, TimeFilter, TimePeriod, TimeSeriesData } from 'src/core/types/financial';
import { CSVParser } from 'src/core/services/csv-parser';
import { aggregateByCountry, aggregateByProfitCenter, aggregateByMonth } from 'src/core/utils/financial-calculations';
...
function createFinancialStore() {
  ...
  function get(store: any): FinancialState {
...
// Derived stores
export const filteredProfitCenters = derived(
  financialStore,
  $financialStore => {
...
export const countryTimeSeries = derived(
  financialStore,
  $financialStore => {
    const csvParser = CSVParser.getInstance();
